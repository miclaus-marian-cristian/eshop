...
public class CategoryEndpointIT {
    ...
    public void shouldReturnStatusOkAnd1TopLevelCategory() {
        ...
        webTestClient.get()
            .uri(ENDPOINT_BASE_URL+"/top-level")
            .exchange()
            .expectStatus().isOk()
    ...
    public void shouldReturn404() {
        ...
        .jsonPath("$.detail").isEqualTo("Resource not found!")
        .jsonPath("$.path").isEqualTo("/api/categories/1")
    ...
    @Test
    public void givenANewCtgryIsBeingAddedWhenTheNewCtgrysNameAlreadyExistsThenReturn409Code() {
    	var ctgryBeingAdded = Category.builder().name("Electronics").build();
